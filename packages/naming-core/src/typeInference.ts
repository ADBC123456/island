...
export function inferVariableType(description: string, requested: VariableType): VariableType {
  ...
  if (/方法|函数|回调|function|handler|validate|format|parse|handle|create|update|delete|remove|fetch|load|save|submit|render|toggle/i.test(description)) return 'function';
